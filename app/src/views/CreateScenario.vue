...
class BlockInfo {
    ...
    constructor(ordernumber, name, label, description, targets, position) {
        ...
        API.execute(API.ROUTE.SCENARIOS+queryParameters["idScenario"], API.METHOD_GET, undefined, API.TYPE_JSON).then(res => {
            ...
            API.execute(API.ROUTE.MACHINES+res.machine.id, API.METHOD_GET, undefined, API.TYPE_JSON).then(machine => {
                ...
                res.steps.forEach(step => {
                    createScenarioBlock(step.name, step.label, step.description, step.targets, step.position);
                });
                resolve();
            }).catch(reject);
        }).catch(reject);
...
function getBlockDiv(id, name, title, desc, targets, pos) {
    let tgs = "";
    // if (targets)
        ...
        <div class="justify-between mb-1">
            ...
            <div class="flex justify-between">
                <div id="shift" class="flex">
                    ...
                    <input type="number" name="pos-x" id="input-stepposx-${id}" value="${pos?pos.x:"0"}" class="input-numbers whitespace-nowrap inline-flex max-w-[72px] text-center p-1 center border-gray-200 rounded-md shadow-sm text-base font-medium text-black bg-gray-50 hover:bg-gray-100">
                </div>
                <div id="shift" class="flex">
                    <p class="text-gray-500 font-base text-lg p-2 mr-4 whitespace-nowrap">Y: </p>
                    <input type="number" name="pos-y" id="input-stepposy-${id}" value="${pos?pos.y:"0"}" class="input-numbers whitespace-nowrap inline-flex max-w-[72px] text-center p-1 center border-gray-200 rounded-md shadow-sm text-base font-medium text-black bg-gray-50 hover:bg-gray-100">
                </div>
                <div id="shift" class="flex">
                    <p class="text-gray-500 font-base text-lg p-2 mr-4 whitespace-nowrap">Z: </p>
                    <input type="number" name="pos-z" id="input-stepposz-${id}" value="${pos?pos.z:"0"}" class="input-numbers whitespace-nowrap inline-flex max-w-[72px] text-center p-1 center border-gray-200 rounded-md shadow-sm text-base font-medium text-black bg-gray-50 hover:bg-gray-100">
                </div>
            </div>
        </div>
    </div>`;
...
let ID_COUNTER = 1;
let scenarioBlocks = [];
function createScenarioBlock(name, title, desc, targets, pos) {
    const newID = ID_COUNTER++;
    const stepZone = document.getElementById("steps-zone");
    const newBlock = getBlockDiv(newID, name, title, desc, targets, pos);
    ...
    setTimeout(() => {
        ...
        const selects = [];
        targets.forEach(target => {selects.push({el: window.indico.addStepTarget(newID, target.id, target.name), id: target.id});});
        updateAvailableTargets();
