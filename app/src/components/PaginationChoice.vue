<template>
    <div v-show="this.obj.showing" id="pagination-parent" class="flex flex-col justify-center absolute top-0 left-0 w-screen h-screen bg-black/[0.4]">
        <div class="p-1 flex flex-col min-w-[50vw] min-h-[50vh] max-w-[70vw] max-h-[70vh] bg_white shadow-lg border border-1 border-gray-200 mx-auto bg-white rounded-lg"> <!-- POPUP -->
            <div class="flex flex-col grow min-h-0">
                <h1 class="text-indigo-600 font-extrabold text-2xl">{{ title }}</h1>
                ...
                <div class="flex flex-col overflow-x-hidden overflow-y-scroll grow border border-1 border-gray-300 rounded-lg bg-indigo-50 min-h-0">
                    <div v-for="el in this.obj.paginationContent[this.obj.pageNumber]" class="flex px-2 py-1 hover:bg-indigo-200 select-none" v-on:click="el.selected = !el.selected">
